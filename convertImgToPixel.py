...
def resize_and_get_specific_pixels(image_path, coordinates, target_size=(256, 256)):
    # Open an image file
    ...
    pixel_matrix = list(img_resized.getdata())

    # Reshape the matrix to match the resized image size
...
# Get and print the specific pixels with grayscale values after resizing
specific_pixels_resized = resize_and_get_specific_pixels(image_path, pixel_coordinates)
for pixel in specific_pixels_resized:
    ...
    print(f"Resized Pixel at ({pixel[0]}, {pixel[1]}): {pixel[2]} grayscale : {grayscale_value}")
